...
/**
 * Mounted alongside linqRoutes on /linqs, so `/:id` there and `/:id/rules` here
...
export const ruleRoutes = new Hono<Env>()
  .get("/:id/rules", idParam, async (c) => {
    ...
    // Load first, so an unknown linq is a 404 rather than an empty list.
  ...
  .put("/:id/rules", idParam, validate("json", rulesPutSchema), async (c) => {
    const { id } = c.req.valid("param")
    const linq = await loadLinq(c.var.db, id)
    assertCanEdit(c.var.principal, linq.ownerId)
    const input = c.req.valid("json")

    // One transaction: a failed insert must never leave the linq with its old
    // rules deleted and no new ones in their place.
    await c.var.db.transaction(async (tx) => {
      await tx.delete(rules).where(eq(rules.linqId, id))
