...
describe("the QR codes API", () => {
  ...
  describe("POST /v1/qr-codes", () => {
    test("creates one with defaults applied, echoing the link's short_url and metadata", async () => {
      ...
      const res = await h.post("/api/v1/qr-codes", author.key, { link_id: link.id })
    ...
    test("an unknown link_id is a 404, even for a viewer", async () => {
      ...
      expect((await h.post("/api/v1/qr-codes", author.key, { link_id: missing })).status).toBe(404)
    ...
    test("permissions follow the link, not the QR code", async () => {
      ...
      expect((await h.post("/api/v1/qr-codes", stranger.key, { link_id: link.id })).status).toBe(403)
      expect((await h.post("/api/v1/qr-codes", viewer.key, { link_id: link.id })).status).toBe(403)
      expect((await h.post("/api/v1/qr-codes", manager.key, { link_id: link.id })).status).toBe(201)
    })
    ...
    test("an archived link is a 409 — the code would be born dead", async () => {
      ...
      expect((await h.post("/api/v1/qr-codes", author.key, { link_id: link.id })).status).toBe(409)
    })

    test("one link can carry more than one QR code", async () => {
      const link = await h.createLink(author.key, domain)
      await h.post("/api/v1/qr-codes", author.key, { link_id: link.id, name: "Poster" })
      await h.post("/api/v1/qr-codes", author.key, { link_id: link.id, name: "Flyer" })

      const res = await h.request(`/api/v1/qr-codes?link_id=${link.id}`, { key: author.key })
      const body = await res.json()
  ...
  describe("GET /v1/qr-codes", () => {
    test("returns the page envelope and narrows by link_id", async () => {
      ...
      await h.post("/api/v1/qr-codes", author.key, { link_id: linkA.id })
      await h.post("/api/v1/qr-codes", author.key, { link_id: linkB.id })
      ...
      const scoped = await (
        await h.request(`/api/v1/qr-codes?link_id=${linkA.id}`, { key: author.key })
      ).json()
    ...
    test("search matches both the QR code's own name and its link's name and slug", async () => {
      ...
      await h.post("/api/v1/qr-codes", author.key, { link_id: named.id, name: "unrelated" })
      await h.post("/api/v1/qr-codes", author.key, { link_id: other.id, name: "unrelated" })
  ...
  describe("PATCH /v1/qr-codes/:id", () => {
    test("updates colours, pattern and name, and bumps updated_at", async () => {
      const link = await h.createLink(author.key, domain)
      const created = await (
        await h.post("/api/v1/qr-codes", author.key, { link_id: link.id })
      ).json()
  ...
  describe("DELETE /v1/qr-codes/:id", () => {
    test("204s, then the row is gone from GET and from the list", async () => {
      ...
      const list = await (
        await h.request(`/api/v1/qr-codes?link_id=${link.id}`, { key: author.key })
      ).json()
