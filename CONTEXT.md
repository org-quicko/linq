...
- **Key**: an API key, and the only principal. Has a name and a Role, owns the Links it creates, and may expire. Shown once, stored hashed. Revoking one is a real delete, and leaves its Links unowned.
- **Role**: `viewer` < `author` < `manager` < `admin`. Carried by a Key. Viewer reads; author also creates and manages the Links its own Key owns; manager also manages any Link; admin also manages Domains and Keys.
