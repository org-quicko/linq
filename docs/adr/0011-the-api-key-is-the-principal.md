...
- **A key's role may not be lowered below the ownership threshold while it still owns links** (amending the note below). Reassigning them (`POST /keys/:id/links/reassign`) or revoking the key outright is the way through; revocation itself stays unconditional, exactly as this ADR requires.

## Amendment: a viewer may still hold links via demotion (2026-09-20)
...
## Amendment: demotion below the ownership threshold is refused while links are owned (2026-09-20)
