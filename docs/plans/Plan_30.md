...
Follows `docs/plans/Plan_29.md` and `docs/plans/Plan_28.md`.

## Context

Currently:
1. Viewing a single link's statistics, settings, rules, and visit log lives at `/links/detail/?id={uuid}`. The link ID is passed as a query parameter because earlier plans (`docs/plans/Plan_6.md`, `docs/plans/Plan_27.md`) assumed that Next.js static export (`output: "export"`) could not support dynamic route segments without pre-rendered IDs.
