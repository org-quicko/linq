...
Leave it running; it serves the API the Admin UI talks to.
...
its own; see [Deploying the Admin UI](#deploying-the-admin-ui) and `docs/adr/0006`.
...
bun run dev:admin  # terminal 2 — Admin UI on :3001
```
...
```bash
bun run test      # server and admin suites, in that order
