...
Follows `plans/Plan_28.md` and `plans/Plan_24.md`.

## Context

In `plans/Plan_24.md`, bot name/type identification (`botLabel`) was introduced using `isbot`'s `findBotMatch(userAgent)` to record specific bot identifiers (e.g., `"googlebot"`, `"facebookexternalhit"`), along with a `bot_label` column on `visits`, a `"botLabel"` dimension in `visitDimensionEnum` and `visit_days` rollups, a `botLabel` query filter, and a "Bot type" filter input and badge in the UI.
