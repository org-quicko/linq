...
Design plans go in `plans/Plan_N.md`, numbered sequentially, and are reviewed
before any code is written from them.

## File

`plans/Plan_N.md` — `N` one higher than the current highest file in `plans/`
(check the directory; don't assume the count from memory). If the work
continues an earlier plan, say so up top the way `plans/Plan_24.md` does:
`Follows plans/Plan_23.md.`
...
`plans/Plan_24.md`'s reasoning about `ua-parser-js` vs. hand-rolled regex for
